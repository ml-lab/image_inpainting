...
def draw_rect(imraw, num_rect, size_max):
  ...
  for i in range(num_rect):
    p_start = p_starts[i, :]
    p_end = (p_start[0] + np.random.randint(5, size_max, 1)[0],
             p_start[1] + np.random.randint(5, size_max, 1)[0])
...
def draw_oval(imraw, num_oval):
  ...
  for i in range(num_oval):
    cv2.ellipse(imraw, tuple(centers[i]), tuple(lens[i]), total_rots[i][0],
                total_rots[i][1], total_rots[i][2], 0, thick[i])
  return imraw
...
def save_mask(num_mask, min_units, max_units, new_mask_path, im_file,
              new_im_path):
  mask_list = []
